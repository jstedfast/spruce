...
static int
mbox_equal (SpruceURL *url0, SpruceURL *url1)
{
...
spruce_provider_module_init (void)
{
	...
	provider->protocol = "mbox";
	provider->name = _("Mbox");
	provider->description = _("Stores mail in the traditional BSD formatted mailboxes.");
	
	provider->object_types[SPRUCE_PROVIDER_TYPE_STORE] =
		spruce_mbox_store_get_type ();
	
	provider->url_hash = (GHashFunc) mbox_hash;
	provider->url_equal = (GCompareFunc) mbox_equal;
